...
<task type="auto">
  ...
  <action>
    First, check if src/styles/ directory already exists:
    - Run: `ls -la src/styles/ 2>/dev/null || echo "Directory does not exist"`
    - If directory exists, check for existing globals.css: `ls src/styles/globals.css 2>/dev/null || echo "File does not exist"`
    - If globals.css exists, back it up: `cp src/styles/globals.css src/styles/globals.css.backup`
  ...
  <verify>ls src/styles/globals.css && grep -E '@import.*tailwindcss|:root|--background|@theme|@layer' src/styles/globals.css</verify>
  <done>src/styles/ directory exists, globals.css is created with @import, :root tokens, @theme mapping, and @layer base</done>
</task>
